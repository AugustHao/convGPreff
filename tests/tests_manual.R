...
obs_N <- reff_data$local$cases

n_jurisdictions <- ncol(obs_N)
n_days <- nrow(obs_N)
#
...
#start infection timeseries a few days before notification
n_days_infection <- n_days + 3
days_infection <- seq_len(n_days_infection)
...
notification_delay_data <- tibble(
    days = sample(days_infection,n_days_infection*100,replace = T),
    delay = ceiling(rnorm(n_days_infection*100,mean = 3,sd = 0.5)) #rep(3,n_days*100)
)
...
#lengthscale inverse gamma prior algebraically calculated alpha and beta based
#on assumed lower and upper bounds of 2,11

# posterior lengthscale tends to higher value, exceeding the upperbound for
# inverse gamma specified, intersting, need to look into this further
...
expected_cases <- convolve(timeseries_matrix = infections,
                           mass_functions = test_full_delay,
                           proportion = car)
...
sqrt_inv_size <- normal(0,
                        ...
                        truncation = c(0, Inf),
                        dim = n_jurisdictions)
sqrt_inv_size <- sweep(zeros(n_days_infection,n_jurisdictions),
                       2,
                       sqrt_inv_size,
                       FUN = "+")
...
size_obs <- size[(3+1):(3+n_days),]

prob_obs <- prob[(3+1):(3+n_days),]
...
case_sims <- calculate(expected_cases[(3+1):(3+n_days),],
                       values = draws,
                       nsim = 1000)
